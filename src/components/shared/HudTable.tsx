...
const colLetter = (i: number) => i < 26 ? COL_LETTERS[i] : COL_LETTERS[Math.floor(i/26)-1] + COL_LETTERS[i%26];
const DEFAULT_COL_W = 120;
const ROW_H = 24;
const HEADER_H = 24;

const FONT_SIZES = [8,9,10,11,12,14,16,18,20,22,24,28,32,36,48,72];
const TEXT_COLORS = ["#000000","#434343","#666666","#999999","#ffffff",
  "#ea4335","#fbbc04","#34a853","#4285f4","#ab47bc","#26a69a","#ef5350","#42a5f5",
  "#ff9900","#ff4500","#d32f2f","#1565c0","#2e7d32","#6a1b9a","#00838f","#f57f17"];
const BG_COLORS = ["transparent","#ffffff","#f8f9fa","#fce8b2","#b7e1cd","#c9daf8","#fce5cd","#ead1dc",
...
function PenaltyCell({ value, colId, canEdit, onSave }: {
  ...
  useEffect(() => { if (step === "reason") setTimeout(() => textRef.current?.focus(), 50); }, [step]);
  ...
  return (
    <div ref={ref} className="relative w-full h-full flex items-center justify-center select-none">
      <span className="text-[12px] font-mono cursor-pointer" style={{ color: clr, fontWeight: num > 0 ? 600 : 400 }}
      ...
      {open && canEdit && (
        <div className="absolute left-0 top-full mt-0.5 z-50 bg-white border border-gray-300 rounded shadow-xl w-52">
          <div className="flex items-center justify-between px-3 py-1.5 bg-gray-50 border-b border-gray-200">
            <span className="text-[11px] font-semibold text-gray-600">{isVerbal?"Уст. предупреждения":"Выговоры"} (0–{max})</span>
            ...
            <div className="flex gap-1.5 p-3">
              {Array.from({length:max+1},(_,i)=>i).map(n => (
                <button key={n} onClick={() => pick(n)}
                  className={`flex-1 h-8 rounded border text-sm font-mono ${n===num?"border-blue-500 bg-blue-50 text-blue-700 font-bold":"border-gray-200 text-gray-600 hover:border-blue-400"}`}>
                  {n}
                </button>
              ))}
            </div>
          ...
          {step === "reason" && (
            <div className="p-3 space-y-2">
              ...
              <textarea ref={textRef} value={reason} onChange={e=>setReason(e.target.value)} rows={2} maxLength={120}
                ...
                placeholder="Причина..." className="w-full px-2 py-1.5 text-[12px] font-mono border border-gray-300 rounded resize-none outline-none focus:border-blue-400"/>
              <div className="flex gap-2">
                <button onClick={confirm} disabled={!reason.trim()} className="flex-1 py-1.5 text-[11px] font-semibold bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-40">Применить</button>
...
// ── Color Picker ───────────────────────────────────────────────
function ColorPicker({ colors, value, onChange, children }: {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(o=>!o)} className="flex items-center gap-0.5 px-1.5 py-1 hover:bg-gray-100 rounded text-gray-600">
        {children}<Icon name="ChevronDown" size={9}/>
      </button>
      {open && (
        <div className="absolute top-full left-0 z-50 bg-white border border-gray-200 rounded shadow-lg p-2 grid grid-cols-8 gap-1" style={{width:180}}>
          {colors.map(c => (
...
// ── Column Header ──────────────────────────────────────────────
function ColHead({ col, ci, width, canEdit, frozen, selected, onSelect, onRename, onDelete, onResizeStart, onSortAsc, onSortDesc }: {
  col: TableColumn; ci: number; width: number; canEdit: boolean; frozen: boolean; selected?: boolean;
  onSelect: () => void; onRename: (id:number,name:string)=>void; onDelete: (id:number)=>void;
  ...
  return (
    <th className={`relative select-none border-r border-b border-gray-300 group p-0
      ...
      <div className="flex items-center justify-center h-full px-1 cursor-pointer gap-0.5" onClick={onSelect}>
        {editing ? (
          ...
          <span className="text-[11px] font-semibold text-gray-600 truncate leading-none" style={{maxWidth:width-20}}
            onDoubleClick={()=>canEdit&&setEditing(true)}>
            {colLetter(ci)} · {col.name}
          </span>
        )}
        {canEdit && !editing && (
          <button className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-gray-600 flex-shrink-0"
      ...
      {/* Resize handle */}
      <div className="absolute right-0 top-0 h-full w-1.5 cursor-col-resize z-10 hover:bg-blue-400"
        onMouseDown={e=>onResizeStart(col.id,e)}/>
      {showMenu && canEdit && (
        <div ref={menuRef} className="absolute left-0 top-full z-50 bg-white border border-gray-200 rounded shadow-xl overflow-hidden min-w-[170px] text-left">
          <button onClick={()=>{onSortAsc();setShowMenu(false);}} className="w-full text-left px-3 py-2 text-[12px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
            <Icon name="ArrowUpAZ" size={12}/> Сортировка А→Я
          </button>
          <button onClick={()=>{onSortDesc();setShowMenu(false);}} className="w-full text-left px-3 py-2 text-[12px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
            <Icon name="ArrowDownZA" size={12}/> Сортировка Я→А
          </button>
          <div className="border-t border-gray-100"/>
          <button onClick={()=>{setEditing(true);setShowMenu(false);}} className="w-full text-left px-3 py-2 text-[12px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
            <Icon name="Pencil" size={12}/> Переименовать
          </button>
          {!AUTO_COLS.has(col.id) && (
            <button onClick={()=>{onDelete(col.id);setShowMenu(false);}} className="w-full text-left px-3 py-2 text-[12px] text-red-500 hover:bg-red-50 flex items-center gap-2 border-t border-gray-100">
...
export default function HudTable({ sheet, canEditCells, canEditStructure, onChange, onPenaltyChange }: HudTableProps) {
  ...
  const addRow = () => push({ ...sheet, rows: [...sheet.rows, { id:Date.now(), cells:Object.fromEntries(sheet.columns.map(c=>[c.id,""])) }] });
  ...
  const renameCol = (id: number, name: string) => push({ ...sheet, columns:sheet.columns.map(c=>c.id===id?{...c,name}:c) });
  const delCol    = (id: number) => push({ ...sheet, columns:sheet.columns.filter(c=>c.id!==id), rows:sheet.rows.map(r=>{const cells={...r.cells};delete cells[id];return{...r,cells};}) });
  ...
  const buildSt = (st: CellStyle): React.CSSProperties => ({
  ...
  // Formula eval
  const evalF = (v: string) => {
    if (!v.startsWith("=")) return v;
    try { return String(Function(`"use strict";return (${v.slice(1).replace(/[^0-9+\-*/.() ]/g,"")})`)()) ; }
    catch { return v; }
  };
  ...
  return (
    <div className="flex flex-col bg-white border border-gray-300 rounded shadow overflow-hidden"
      ...
      tabIndex={0} onKeyDown={onKey} ref={containerRef} outline-none>
      ...
      <div className="flex items-center gap-0.5 px-2 border-b border-gray-200 bg-white flex-wrap" style={{minHeight:38}}>
        ...
        <ColorPicker colors={TEXT_COLORS} value={selSt.color??"#000000"} onChange={c=>applySt("color",c)}>
          <span className="text-[13px] font-bold leading-none" style={{color:selSt.color??"#000",borderBottom:`2px solid ${selSt.color??"#ea4335"}`}}>А</span>
        </ColorPicker>

        {/* BG color */}
        <ColorPicker colors={BG_COLORS} value={selSt.bg??"transparent"} onChange={c=>applySt("bg",c)}>
          <span className="relative flex items-center"><Icon name="Paintbrush" size={13}/>
      ...
      {showAddCol && canEditStructure && (
        <div className="flex items-center gap-2 px-3 py-1.5 border-b border-gray-200 bg-gray-50 flex-wrap">
          <input value={newColName} onChange={e=>setNewColName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addCol()}
      ...
      <div className="overflow-auto" style={{maxHeight:460}}>
        <div style={{transform:`scale(${zoom/100})`,transformOrigin:"top left",width:`${10000/zoom}%`}}>
          <table className="border-collapse" style={{tableLayout:"fixed",width:"max-content"}}>
            <thead className="sticky top-0 z-20">
              <tr style={{height:HEADER_H}}>
                ...
                {sheet.columns.map((col,ci)=>(
                  <ColHead key={col.id} col={col} ci={ci} width={getW(col)} canEdit={canEditStructure&&!AUTO_COLS.has(col.id)}
            ...
            <tbody>
              {sheet.rows.length===0 && (
                <tr><td colSpan={sheet.columns.length+2} className="px-4 py-10 text-center text-[12px] text-gray-400">
                    ...
                    {sheet.columns.map((col,ci)=>{
                      ...
                      return (
                        <td key={col.id}
                          ...
                          style={{width:getW(col),minWidth:getW(col),maxWidth:getW(col),height:ROW_H,
                          ...
                          {isPen ? (
                            <PenaltyCell value={raw} colId={col.id} canEdit={canEditCells}
                              onSave={(v,reason)=>{
                                ...
                                if(onPenaltyChange) onPenaltyChange(nick,col.id===COL_ID_VERBAL?"verbal":"reprimand",parseInt(v),reason);
                              }}/>
                          ) : isEd ? (
                            <input ref={inputRef} value={editValue} onChange={e=>setEditValue(e.target.value)}
                              onBlur={commitEdit}
                            ...
                            <div className="w-full h-full flex items-center px-2 overflow-hidden" style={buildSt(st)}>
